...
`portfolio_ids_internal_pd` file is read below, validated for `internal_pd`, and
reduced to `portfolio_terms` (`company_id`, `term`, `exposure_value_usd`). Those
loan terms and exposures are what turn each parameter run into a bank-specific,
EAD-weighted result — so swapping in your own `portfolio_ids` (with `internal_pd`)
is what makes every chart below reflect *your* book.
...
Read the EL bps delta as the bank-impact summary of this one variant. The
sensitivity sections above tell you how that number would move if you chose a
different shock year, IAM, or ambition tier. For deeper plot references and the
full methodology, see [`pd-el-integration`](bank_4_pd-el-integration.html).
...
- [`pd-el-integration`](bank_4_pd-el-integration.html) — the full PD/EL integration
  methodology and the EL bps KPI surfaced in the closing section.
