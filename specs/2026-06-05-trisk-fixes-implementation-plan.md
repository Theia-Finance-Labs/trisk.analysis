...
- **Full runner: NOT deprecated; removed from bank vignettes (2026-06-06, superseding the earlier deprecation).** `run_trisk_on_portfolio` is a valid technology-resolved runner — it is simply not part of the *bank* workflow. The runtime `.Deprecated()` was reverted, roxygen restored to a neutral technology-resolved description (kept in the pkgdown reference), and the deprecation test wrappers (`expect_warning`/`suppressWarnings`) reverted to plain calls. The function is dropped from the bank vignettes (bank_2, bank_4), which now run only `run_trisk_on_simple_portfolio`. The per-technology NPV/exposure/PD/EL plots in bank_2 and the PD/EL integration in bank_4 are fed from the simple runner's `portfolio_results_tech_detail` (exposing `exposure_at_default` as `exposure_value_usd` where a plot aggregates notional exposure). Verified by knitting both vignettes against dev code.
