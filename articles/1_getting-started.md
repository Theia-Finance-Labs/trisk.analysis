...
1.  `vignette("inputs-and-outputs")` — what TRISK consumes (assets,
    scenarios, financial features, carbon prices) and what it produces
    (NPV and PD results).
2.  `vignette("run-on-a-portfolio")` — run TRISK on a bank loan book and
    map the shocks onto your exposures.
3.  `vignette("pd-el-integration")` — blend TRISK shocks with the bank’s
    own PD/EL to get climate-adjusted expected loss.
4.  `vignette("sensitivity-analysis")` — sweep scenario assumptions to
    see how results move across baselines and shock scenarios.
...
Start with `vignette("inputs-and-outputs")` to understand the data TRISK
needs before running it on your own portfolio.
