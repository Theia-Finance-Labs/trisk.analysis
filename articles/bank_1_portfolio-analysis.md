# Bank workflow 1: Portfolio analysis — start here
...
outputs](bank_2_inputs-and-outputs.md), [Run on a
portfolio](bank_3_run-on-a-portfolio.md), [Sensitivity
analysis](bank_4_sensitivity-analysis.md), [PD & EL
integration](bank_5_pd-el-integration.md)) go deeper; start here.
...
For the [Sensitivity analysis](bank_4_sensitivity-analysis.md) and [PD &
EL integration](bank_5_pd-el-integration.md) workflows, add an
`internal_pd` column (your own PD per company, in `[0, 1]`) — that is
the `portfolio_ids_internal_pd` variant those vignettes use.
...
- **[Inputs and outputs](bank_2_inputs-and-outputs.md)** — every input
  column and every output table, in detail.
- **[Run on a portfolio](bank_3_run-on-a-portfolio.md)** — the simple vs
  full runners, exposure allocation, and the plotting helpers.
- **[Sensitivity analysis](bank_4_sensitivity-analysis.md)** — sweep
  shock year, IAM and ambition, read through a bank-impact lens.
- **[PD & EL integration](bank_5_pd-el-integration.md)** — recombine
  TRISK PDs with your internal PDs and translate to expected-loss basis
  points.
